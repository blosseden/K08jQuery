...
/*
 $.get()
 	...
 	사용법
 		...
 		- url : 정보를 요청 할 경로
 		- param : 서버로 전송 할 파라미터로 json형태로 기술한다.
 		- success(data) : 요청이 성공 했을 때 실행 되는 콜백(callback)메소드.
 			콜백메소드가 호출 될 때 전달되는 파라미터를 통해 성공 혹은
 			실패 여부를 판단 할 수 있다.
 		※ 서버로 요청시 전송 할 값(파라미터)이 없는 경우에는 생략 가능함
 */
$(function(){
	...
	$('#btnJSP').click(function(){
     ...
     -> 해당 메소드를 이용하면 form 하위의 모든 요소에 대해
        JSON으로 조립해준다. 단, input 요소의 name 속성값이
        Key로 사용된다는 것에 주의하자.
     */
});
